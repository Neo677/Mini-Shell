...
//         }
//         if (**ctx.input == '!')
//             return (*last_exit_status = ctx.exit_status = 1, 0);
//         else if (**ctx.input == '|' || **ctx.input == '>'
	// || **ctx.input == '<')
//         {
...
// 				{
// 					ft_printf("[ERROR] Échec de l'ajout de la redirection :
						// %s\n", file);
// 					ft_free_token(*head);
// 					*head = NULL;
// 					ft_free_commande_lst(cmd_lst);
...
// 			{
// 				ft_printf("[ERROR] Impossible d'ajouter la variable en argument :
					// %s\n", var_name);
// 				ft_free_token(*head);
// 				*head = NULL;
// 				ft_free_commande_lst(cmd_lst);
